OpenHelpFile(Keyword := "") {
    ...
    ; Object methods
    } Else If (Keyword ~= "i)^(Obj)?(InsertAt|RemoveAt|Push|Pop|Delete|MinIndex|MaxIndex|Length|SetCapacity|GetCapacity|GetAddress|NewEnum|HasKey|Clone|ObjRawSet)$") {
    ...
    ; GUI control types
    } Else If (Keyword ~= "i)^(Button|CheckBox|ComboBox|Custom|DateTime|DropDownList|Edit|GroupBox|Link|ListBox|MonthCal|Picture|Radio|Slider|StatusBar|Tab(2)?|Text|UpDown|ActiveX)$") {
        HTMLPage := "GuiControls.htm#" . Keyword

    } Else If (Keyword = "Hotkey" || Keyword = "Progress") {
